...
class ConsumerUsage:
                                ...
                                if isinstance(json_dict["table"], list):
                                    for table in json_dict["table"]:
                                            for col in table["column"]:
                                                ...
                                                data["schema"]= table["schema"] if table["schema"] else data["SCHEMA_NAME"]
                                    ...
                                    if isinstance(json_dict["table"]["column"], list):
                                        for col in json_dict["table"]["column"]:
                                            data["db"] = json_dict["table"]["database"] if json_dict["table"]["database"] else data[
                                                        "DATABASE_NAME"]
                                            data["schema"] = json_dict["table"]["schema"] if json_dict["table"]["schema"] else data["SCHEMA_NAME"]
                                        ...
                                        data["db"] = json_dict["table"]["database"] if json_dict["table"][
                                            "database"] else data[
                                            "DATABASE_NAME"]
                                        data["schema"] = json_dict["table"]["schema"] if json_dict["table"][
                                        ...
                                        response = requests.post(url, headers=headers, data=payload)
                                        self.json_to_parquet_to_hive(self, payload, collection_name,
                                                                is_file=True)
    ...
    def insert_document(self,arango_url,database_name,collection_name,json_document,keyToCheck):
        ...
        self.create_hive(self.hive_serer,self.hive_port,database_name)
    ...
    def json_to_parquet_to_hive(self,json_input, table_name,
                                is_file=True):
        parquet_dir=f'{self.database}/{table_name}'
        ...
        # Convert start_time to date if it's not already in date format
        df['start_time'] = pd.to_datetime(df['start_time']).dt.date

        parquet_file = os.path.join(parquet_dir, json_base_name.replace('.json', '.parquet'))
        df.to_parquet(parquet_file, engine='pyarrow', index=False)
        self.log(f'Converted JSON input to {parquet_file}')
        ...
        for col, dtype in zip(columns, dtypes):
            if col != 'start_time':  # Exclude partition column from main column list
        ...
        # Connect to Hive
        conn = hive.Connection(host=hive_server, port=10000, username='hive')
        cursor = conn.cursor()

        # Create the database if it doesn't exist
        self.create_hive(self.hive_serer,self.hive_port,self.database)
        ...
        self.log(f"Table {table_name} is ready in database {self.database}")

        # Get unique partition dates from the DataFrame
        partition_dates = df['start_time'].unique()
        ...
        for partition_date in partition_dates:
            ...
            cursor.execute(f"ALTER TABLE {table_name} ADD IF NOT EXISTS PARTITION (start_time='{partition_date_str}')")

            # Load data for the specific partition
            load_data_query = f"""
            LOAD DATA LOCAL INPATH '{parquet_file}' INTO TABLE {table_name} PARTITION (start_time='{partition_date_str}')
            """
            cursor.execute(load_data_query)
            self.log(
                f"Loaded data from {parquet_file} into Hive table {table_name} partition (start_time='{partition_date_str}')")
...
def run_api():
    with open('Config.json', 'r') as f:
        global config, server, usr, pwd, engine, cnxn, arangoDBUrlServer, envConnString, kafka_broker, GspUrl,list_of_tables,gspUrl,dbvVendor,hive_server,hive_port
        try:
