                        ...
                        if isinstance(json_dict["table"], list):
                            for table in json_dict["table"]:
                                for col in table["column"]:
                                    ...
                                    try:
                                        self.json_to_parquet_to_hive(payload, collection_name, database_name, "START_TIME",
                                                                     is_file=True)
                                    except Exception as e:
                                        self.log.error(e)
                            ...
                            if isinstance(json_dict["table"]["column"], list):
                                for col in json_dict["table"]["column"]:
                                    data["db"] = json_dict["table"]["database"] if json_dict["table"]["database"] else \
                                    data[
                                        "DATABASE_NAME"]
                    ...
                    if 'error' not in json_dict:
                        if "table" in json_dict:
                            if isinstance(json_dict["table"], list):
                                for table in json_dict["table"]:
                                    for col in table["column"]:
                                        if isinstance(col, list):
                                            for clm in col:
                                                data["db"] = table["database"].replace("[", "").replace("]",
                                                                                                        "") if "database" in table else \
                                                data["databasename"]
                                                data["schemaName"] = table["schema"].replace("[", "").replace("]",
                                                                                                              "") if "schema" in table else 'dbo'
                                                data["tablename"] = table["name"].split(".")[-1].replace("[",
                                                                                                         "").replace(
                                                    "]", "")
                                                ...
                                                data["QUERY_ID"] = self.generate_md5_hashTxt(
                                                    f'{data["object_id"]}{data["event_time"]}')

                                                payload = json.dumps(data)
                                                response = requests.post(url, headers=headers, data=payload)
                                                try:
                                                    self.json_to_parquet_to_hive(payload, collection_name,
                                            ...
                                            data["db"] = table["database"] if "database" in table else data[
                                                "databasename"]
                                ...
                                if isinstance(json_dict["table"]["column"], list):
                                    for col in json_dict["table"]["column"]:
                                        data["db"] = json_dict["table"]["database"] if "database" in json_dict[
                                            "table"] else data["databasename"]
                                        data["schemaName"] = json_dict["table"]["schema"] if "schema" in json_dict[
                                            "table"] else 'dbo'
                                        data["tablename"] = json_dict["table"]["name"].split(".")[-1]
                                        data["columnName"] = col["name"]
                                    ...
                                    data["db"] = json_dict["table"]["database"] if "database" in json_dict["table"] and \
                                                                                   json_dict["table"]["database"] else \
                                    data["databasename"]
                                    data["schemaName"] = json_dict["table"]["schema"] if "schema" in json_dict[
                                        "table"] else 'dbo'
                                    data["tablename"] = json_dict["table"]["name"].split(".")[-1]
    ...
    def json_to_parquet_to_hive(self, json_input, table_name, database_name, dateField, is_file=True):
        ...
        if is_file:
            try:
                ...
                json_dict["tablename"]= json_dict["tablename"][0]
        ...
        for partition_date in partition_dates:
            ...
            try:
                ...
                if json_dict["eventname"]=="sql_batch_starting":
                    load_data_query = f"""
                    LOAD DATA INPATH '{hdfs_path}' INTO TABLE {table_name}_TMP PARTITION ({dateField}='{partition_date_str}')
                    """
                    ...
                    cursor.execute(load_data_query)
                    self.log.info(
                        f"Loaded data from {hdfs_path} into Hive table {table_name}_TMP partition ({dateField}='{partition_date_str}')")
                else:
